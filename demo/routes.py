...
from core.utils import advance_demo_date, reset_demo_date, get_today_date
...
@demo_bp.route("/demo/reset-solve", methods=["POST"])
@login_required
def reset_solve():
    # 데모: 현재 날짜의 내 풀이 기록 삭제 → 같은 문제를 처음부터 다시 풀 수 있게.
    # (정답/오답/실패 화면을 반복 시연하려고 사용). today()가 다음 접속 시 새 풀이를 만듦.
    date = get_today_date()
    db.solves.delete_one({"user_id": g.user_id, "date": date})
    return redirect(url_for("quiz.today"))
